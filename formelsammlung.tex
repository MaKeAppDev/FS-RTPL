...
\begin{multicols*}{4}
\fstitle{Real Time Programming Languages}
...
\item statements take time if they say so: await..., every...
\end{itemize}
...
easy to analyze & semantically not easy for the programmer\\
...
\cooknumbox{Logical Correctness Check}{
...
\end{itemize}
\cooknumbox{Module Checking in Esterel}{
...
\cooknumbox{WCET Analysis using IPET/ILP}{
...
\end{itemize}
\emphbox{WCET = $\sum\limits_{i=1}^N\sum\limits_{j=1}^{n_i} c_{i,j}^\text{hit}x_{i,j}^\text{hit} + c_{i,j}^\text{miss}x_{i,j}^\text{miss}$}

\subsection{WCET analysis with caches}
\emphbox{WCET = $\underset{x^\text{hit}, x^\text{miss}}{\text{max}}\sum\limits_{i=1}^N c_i^\text{hit}x_i^\text{hit} + c_i^\text{miss}x_i^\text{miss}$}
...
\section{Ada}
\sectionbox{
\emphbox{Scheduled Model Language}
...
\emphbox{$u = \sum\limits_{i = 1}^{n} \frac{e_i}{P_i} \le n \left(\sqrt[n]{2} - 1 \right)$}
